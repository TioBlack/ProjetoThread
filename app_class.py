...
#############################################    HELP FUNCTIONS     ####################################################
    ...
    def empacotadores_settings(self):
        # 1° EMPACOTADOR
        if len(empacotador.id_list) > 0:
            if empacotador.id_list[0]:
                self.draw_text(str(empacotador.id_list[0]), self.screen, [WIDTH // 2 + 150, HEIGHT - 460],
                                         START_TEXT_SIZE, (170, 132, 58), START_FONT, centered=True)
                self.draw_text(str(empacotador.te), self.screen, [WIDTH // 2 + 153, HEIGHT - 360],
                                   START_TEXT_SIZE - 4,
                                   (170, 132, 58), START_FONT, centered=True)
                self.draw_text("-", self.screen, [WIDTH // 2 + 130, HEIGHT - 360], START_TEXT_SIZE,
                                   (170, 132, 58),
                                   START_FONT, centered=True)
                self.draw_text("+", self.screen, [WIDTH // 2 + 175, HEIGHT - 360], START_TEXT_SIZE,
                                   (170, 132, 58),
                                   START_FONT, centered=True)
                self.screen.blit(EMPACOTADOR_RED, (WIDTH // 2 + 77, HEIGHT - 450))
                # 2° EMPACOTADOR
                if len(empacotador.id) > 1:
                    self.draw_text(str(empacotador.id_list[1]), self.screen, [WIDTH // 2 + 260, HEIGHT - 460],
                                       START_TEXT_SIZE, (170, 132, 58), START_FONT, centered=True)
                    self.draw_text(str(empacotador.te), self.screen, [WIDTH // 2 + 261, HEIGHT - 360],
                                       START_TEXT_SIZE - 4,
                                       (170, 132, 58), START_FONT, centered=True)
                    self.draw_text("-", self.screen, [WIDTH // 2 + 238, HEIGHT - 360], START_TEXT_SIZE,
                                       (170, 132, 58),
                                       START_FONT, centered=True)
                    self.draw_text("+", self.screen, [WIDTH // 2 + 283, HEIGHT - 360], START_TEXT_SIZE,
                                       (170, 132, 58),
                                       START_FONT, centered=True)
                    self.screen.blit(EMPACOTADOR_RED, (WIDTH // 2 + 185, HEIGHT - 450))
                    # 3° EMPACOTADOR
                    if len(empacotador.id) > 2:
                        self.draw_text(str(empacotador.id_list[2]), self.screen, [WIDTH // 2 + 150, HEIGHT - 300],
                                           START_TEXT_SIZE, (170, 132, 58), START_FONT, centered=True)
                        self.draw_text(str(empacotador.te), self.screen, [WIDTH // 2 + 151, HEIGHT - 200],
                                           START_TEXT_SIZE - 4,
                                           (170, 132, 58), START_FONT, centered=True)
                        self.draw_text("-", self.screen, [WIDTH // 2 + 128, HEIGHT - 200], START_TEXT_SIZE,
                                           (170, 132, 58),
                                           START_FONT, centered=True)
                        self.draw_text("+", self.screen, [WIDTH // 2 + 173, HEIGHT - 200], START_TEXT_SIZE,
                                           (170, 132, 58),
                                           START_FONT, centered=True)
                        self.screen.blit(EMPACOTADOR_RED, (WIDTH // 2 + 75, HEIGHT - 290))
                        # 4° EMPACOTADOR
                        if len(empacotador.id) > 3:
                            self.draw_text(str(empacotador.id_list[3]), self.screen, [WIDTH // 2 + 260, HEIGHT - 300],
                                               START_TEXT_SIZE, (170, 132, 58), START_FONT, centered=True)
                            self.draw_text(str(empacotador.te), self.screen, [WIDTH // 2 + 261, HEIGHT - 200],
                                               START_TEXT_SIZE - 4,
                                               (170, 132, 58), START_FONT, centered=True)
                            self.draw_text("-", self.screen, [WIDTH // 2 + 239, HEIGHT - 200],
                                               START_TEXT_SIZE,
                                               (170, 132, 58),
                                               START_FONT, centered=True)
                            self.draw_text("+", self.screen, [WIDTH // 2 + 282, HEIGHT - 200],
                                               START_TEXT_SIZE,
                                               (170, 132, 58),
                                               START_FONT, centered=True)
                            self.screen.blit(EMPACOTADOR_RED, (WIDTH // 2 + 185, HEIGHT - 290))
                            # 5° EMPACOTADOR
                            if len(empacotador.id) > 4:
                                self.draw_text(str(empacotador.id_list[4]), self.screen,
                                                   [WIDTH // 2 + 150, HEIGHT - 140],
                                                   START_TEXT_SIZE, (170, 132, 58), START_FONT, centered=True)
                                self.draw_text(str(empacotador.te), self.screen,
                                                   [WIDTH // 2 + 150, HEIGHT - 40],
                                                   START_TEXT_SIZE - 4,
                                                   (170, 132, 58), START_FONT, centered=True)
                                self.draw_text("-", self.screen, [WIDTH // 2 + 129, HEIGHT - 40],
                                                   START_TEXT_SIZE,
                                                   (170, 132, 58),
                                                   START_FONT, centered=True)
                                self.draw_text("+", self.screen, [WIDTH // 2 + 171, HEIGHT - 40],
                                                   START_TEXT_SIZE,
                                                   (170, 132, 58),
                                                   START_FONT, centered=True)
                                self.screen.blit(EMPACOTADOR_RED, (WIDTH // 2 + 74, HEIGHT - 130))
        else:
            pass
